...
public:
    ...
    {
        // 1) Create or reuse a persistent ESP-IDF project directory (for incremental builds)
        File tempProjRoot(outdir);
        if (! tempProjRoot.exists()) tempProjRoot.createDirectory();
        auto projectName = name.isNotEmpty() ? name : String("plugdata_esp_export");
        ...
        // Top-level CMakeLists.txt with minimal build
        {
            String cmakeTop;
        ...
        // 2) Run hvcc to generate Heavy C into project (force name to Untitled)
        if (exportingView)
        ...
        // 3) Patch HvMessage.c formatting inside project c/
        File targetDir = cDir;
        ...
        // 4) Source ESP-IDF, build and flash (incremental, ccache enabled)
        if (exportingView) exportingView->logToConsole("ESP32: preparando ambiente, build incremental e flash...\n");
        ...
        argv.add("-lc");
        argv.add(String(". \"$HOME/esp/esp-idf/export.sh\" >/dev/null 2>&1; ")
        ...
        // Optional: log bin path if present in project
        {
